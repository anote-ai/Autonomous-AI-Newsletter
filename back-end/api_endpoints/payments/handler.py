...
from database.db import user_id_for_email, add_subscription, delete_subscription, stripe_subscription_for_user, config_for_payment_tiers, user_has_free_trial, stripe_customer_for_user, next_anchor_time_for_user
import stripe
...
#     free_trial_code = 'FREE123'
#     session = {'id': 'subscription1'}
#     user_id = 1
#     customer_id = 'cus_1234567890'
#     payment_plan = PaidUserStatus.STANDARD_TIER
#     is_free_trial = True
#     start_time = datetime.now()
#     subscription = {'id': 'subscription1'}
#     end_time = datetime.now()
#     is_update = False
#     new_subscription = {'id': 'subscription2'}

#     # Call and print the results of the functions

#     # print("Test: user_has_free_trial()")
#     # result = user_has_free_trial(userEmail, free_trial_code)
#     # print(result)

#     # print("\nTest: add_subscription()")
#     # add_subscription(session, user_id, customer_id, payment_plan, is_free_trial, start_time)

#     # print("\nTest: delete_subscription()")
#     # delete_subscription(subscription, end_time, is_update)

#     # print("\nTest: update_subscription()")
#     # update_subscription(new_subscription, user_id, customer_id, payment_plan, start_time)

#     print("\nTest: stripe_subscription_for_user()")
#     result = stripe_subscription_for_user(userEmail)
#     print(result)

#     print("\nTest: stripe_customer_for_user()")
#     result = stripe_customer_for_user(userEmail)
#     print(result)

#     print("\nTest: next_anchor_time_for_user()")
#     result = next_anchor_time_for_user(user_id)
#     print(result)

#     print("\nTest: refresh_credits()")
#     result = refresh_credits(userEmail)
#     print(result)
    ...
    if existing_subscription_id:
        ...
        try:
            ...
            checkout_session = stripe.checkout.Session.create(
                ...
                mode='subscription',
                success_url="https://newsletter.anote.ai",
                cancel_url="https://newsletter.anote.ai",
                metadata={
                    'user_id': user_id
                },
...
def StripeWebhookHandler(request, event):
    ...
    if eventType == 'checkout.session.completed':
        ...
        if cancellation_reason == "cancellation_requested":
            ...
            refresh_credits(user_email)
        # Commented out upgrade/downgrade code
        # else:
        ...
        #         # else:
        #         current_items = event['data']['object']['items']['data']
        #         current_price = current_items[0]['plan']['amount']

        #         # Get the previous plan amount; default to 0 if not available (indicating a free trial or other reason)
        #         previous_items = event['data']['previous_attributes'].get('items', {}).get('data', [{}])
        #         previous_price = previous_items[0].get('plan', {}).get('amount', 0)

        #         start_time = None
        #         if current_price < previous_price:
        #             start_time = next_anchor_time_for_user(user_id)

        #         update_subscription(subscription, customer_id, payment_plan, start_time)
        #         user_email = user_email_for_id(user_id)
        #         refresh_credits(user_email)
