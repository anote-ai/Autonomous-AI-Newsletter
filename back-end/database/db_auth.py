...
def verifyAuthForSearch(user_email):
    ...
    cursor.execute('SELECT COUNT(*) FROM searches c JOIN users p ON c.user_id=p.id WHERE p.email=%s AND c.created > DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 1 DAY)', [user_email])
    ...
    if count["COUNT(*)"] > threshold:
        access_invalid = True
    conn.close()
    return not access_invalid

def profile_lists_access_invalid(user_id, profile_list_id):
    conn, cursor = get_db_connection()
    cursor.execute('SELECT COUNT(*) FROM lists WHERE id=%s AND user_id=%s', [profile_list_id, user_id])
    count = cursor.fetchone()
    access_invalid = True
    if count["COUNT(*)"] != 0:
        access_invalid = False
...
def profiles_multi_access_invalid(user_id, profile_ids):
    conn, cursor = get_db_connection()
    access_invalid = False
    for profile_id in profile_ids:
        cursor.execute('SELECT COUNT(*) FROM profiles c JOIN unlockedProfiles p ON p.profile_id=c.id WHERE c.id=%s AND p.user_id=%s', [profile_id, user_id])
        count = cursor.fetchone()
        if count["COUNT(*)"] == 0:
            access_invalid = True
    conn.close()
...
def sequence_texts_access_invalid(user_id, sequence_text_id):
    conn, cursor = get_db_connection()
    cursor.execute('SELECT COUNT(*) FROM sequenceTexts c JOIN sequences p ON p.id=c.sequence_id JOIN lists pp ON pp.id=p.list_id WHERE c.id=%s AND pp.user_id=%s', [sequence_text_id, user_id])
    count = cursor.fetchone()
    access_invalid = True
    if count["COUNT(*)"] != 0:
        access_invalid = False
...
def sequence_texts_multi_access_invalid(user_id, sequence_text_ids):
    conn, cursor = get_db_connection()
    access_invalid = False
    for sequence_text_id in sequence_text_ids:
        cursor.execute('SELECT COUNT(*) FROM sequenceTexts c JOIN sequences p ON p.id=c.sequence_id JOIN lists pp ON pp.id=p.list_id WHERE c.id=%s AND pp.user_id=%s', [sequence_text_id, user_id])
        count = cursor.fetchone()
        if count["COUNT(*)"] == 0:
            access_invalid = True
